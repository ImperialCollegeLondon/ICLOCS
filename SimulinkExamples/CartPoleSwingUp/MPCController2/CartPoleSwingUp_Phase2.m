function [problem,guess] = CartPoleSwingUp_Phase2
...
InternalDynamics=@CartPoleSwingUp_Dynamics_Internal_Phase2;
SimDynamics=@CartPoleSwingUp_Dynamics_Sim_Phase2;
...
% Settings file
problem.settings=@settings_CartPoleSwingUp_Phase2;
...
problem.states.x0=[-0.791692281747429,0.513669099112361,0.776322606038674,3.05545941613949];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[-0.791692281747429,0.513669099112361,0.776322606038674,3.05545941613949];
problem.states.x0u=[-0.791692281747429,0.513669099112361,0.776322606038674,3.05545941613949];
...
problem.states.xfl=[0 0 -2 deg2rad(170)]; 
problem.states.xfu=[0 0 2 deg2rad(190)];

% Guess the state trajectories with [x0 xf]
guess.states(:,1)=[0 0];
guess.states(:,2)=[0 0];
guess.states(:,3)=[1 1];
guess.states(:,4)=[pi pi];
...
stageCost = 0.01*u(:,1).*u(:,1)+100*(max(deg2rad(179)-x(:,4),0)).^2+100*(max(x(:,4)-deg2rad(181),0)).^2+10*(max(0.95-x(:,3),0)).^2+10*(max(x(:,3)-1.05,0)).^2;
