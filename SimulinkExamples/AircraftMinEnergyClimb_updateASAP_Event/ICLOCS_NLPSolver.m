...
if simtime>(solution.t_ref+30)
    re_opt=1;
end
if simtime>(solution.t_ref+solution.tf)
    re_opt=1;
end
...
if (simtime~=0 && ~re_opt) || mod(simtime,tstep)~=0 % For time instances without re-optimization
    ...
    if simtime~=0
        ...
        crt_time=simtime-solution.t_ref; % Current time in the frame of previous solution
        tfpu=[solution.tf-opt_step;computation_time;speval(solution.Up,1,crt_time)] ; % Return the interpolated solution
    else
