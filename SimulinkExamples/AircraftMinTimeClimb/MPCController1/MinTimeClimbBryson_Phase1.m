function [problem,guess] = MinTimeClimbBryson_Phase1
%MinTimeClimbBryson_Phase1 - Supersonic Aircraft Minimum Time Climb Problem
%
...
%
% Syntax:  [problem,guess] = MinTimeClimbBryson_Phase1
%
% Outputs:
...
InternalDynamics=@MinTimeClimbBryson_Dynamics_Internal_Phase1;
SimDynamics=[];
...
% Settings file
problem.settings=@settings_MinTimeClimbBryson_Phase1;
...
problem.states.xfl=[altmin speedmin fpamin massmin]; 
problem.states.xfu=[altmax speedmax fpamax massmax]; 
...
problem.data.altf = altf;
problem.data.speedf = speedf;
problem.data.fpaf = fpaf;
...
boundaryCost=tf+100*(xf(1)-vdat.altf).^2+100*(xf(2)-vdat.speedf).^2+1000*(xf(3)-vdat.fpaf).^2;
