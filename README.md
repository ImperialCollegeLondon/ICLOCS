...
From a fresh clone with MATLAB R2019a or later, run:

```bash
matlab -batch "installIclocs"
```

Or from the repository root in MATLAB:

```matlab
installIclocs
```

The installer adds the ICLOCS source folders to the MATLAB path, checks optional solver dependencies, and builds the `DoubleIntegratorTracking` example problem without solving it.
