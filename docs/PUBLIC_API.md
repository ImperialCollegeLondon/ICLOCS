...
One-command setup helper for first-time users.
...
report = installIclocs('runSmokeTest',false);
```

Inputs:

- Name-value options:
  - `'savePath'`: save the updated MATLAB path with `savepath`. The default is `false`.
  - `'runSmokeTest'`: build a small example problem without solving it. The default is `true`.
  - `'example'`: example problem used for the smoke check. The default is `'DoubleIntegratorTracking'`.
  - `'quiet'`: suppress progress output. The default is `false`.

Outputs:

- `report`: structure containing path, dependency, solver-availability, and smoke-check status.

Side effects and assumptions:

- Adds the repository `tools` folder, `src` tree, and selected example folder to the MATLAB path.
- Does not install third-party solvers automatically.
- Does not solve an optimal-control problem during the smoke check.

## `iclocsDoctor`

Diagnostic helper for path and optional dependency checks.
...
- `repoRoot`: optional path to the ICLOCS repository root.
- `'quiet'`: optional flag that suppresses printed output.

Outputs:

- `report`: structure containing MATLAB version status, path status, optional dependency checks, and solver availability.

Side effects and assumptions:

- Reports missing solver interfaces, ADiGator, and Simulink as diagnostic information.
- Does not modify the MATLAB path by itself.
