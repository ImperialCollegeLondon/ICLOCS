...
From a terminal in the repository root with MATLAB R2019a or later:

```bash
matlab -batch "installIclocs"
```
...
`installIclocs` adds the ICLOCS source folders and `tools` folder to the MATLAB path, checks for supported solver dependencies, and builds the `DoubleIntegratorTracking` example problem without running a solve. This catches path and dependency issues without requiring IPOPT, WORHP, or Optimization Toolbox to be installed first.

By default, the installer does not permanently edit your MATLAB path. To save the path after setup:

```matlab
installIclocs('savePath',true)
```

Third-party solvers are not installed automatically because their MATLAB interfaces and licenses vary by platform. The dependency report tells you which supported solver interfaces MATLAB can already see.

To run the dependency report again later:
