function [ auxdata ] = transcribeResErrorAnalysis(t_list,options,dataNLP )
%transcribeResErrorAnalysis - transcribe the problem in preparation for error analysis
%of integrated residual
%
% Syntax:  [ auxdata ] = transcribeResErrorAnalysis(t_list,options,dataNLP )
%
...
switch dataNLP.options.discretization
    case{'globalLGR','hpLGR'} % p/hp Transcription Method
        ...
        if dataNLP.options.scaling
            XscaleMat=1./repmat( dataNLP.data.Xscale, M+1, 1 );
            auxdata.XscaleMat=diag(XscaleMat(:));
            auxdata.XshiftMat=repmat( dataNLP.data.Xshift, M+1, 1 );
            UscaleMat=1./repmat( dataNLP.data.Uscale, M, 1 );
            auxdata.UscaleMat=diag(UscaleMat(:));
            auxdata.UshiftMat=repmat( dataNLP.data.Ushift, M, 1 );
            if isfield(dataNLP.data,'Pscale')
                PscaleMat=1./repmat( dataNLP.data.Pscale, M, 1 );
                auxdata.PshiftMat=repmat( dataNLP.data.Pshift, M, 1 );
                auxdata.PscaleMat=diag(PscaleMat(:));
            end
            auxdata.ResNormScale=dataNLP.data.Xscale.*dataNLP.data.resNormCusWeight;
            ResNormScaleMat=repmat(auxdata.ResNormScale , nps, 1 );
            auxdata.ResNormScaleMat=diag(ResNormScaleMat(:));
        else
...
%         DxHS_p1=sparse(diag(4*ones(M,1))+diag(-8*ones(M-1,1),1)+diag(4*ones(M-2,1),2));
        ...
        if dataNLP.options.scaling
            XscaleMat=1./repmat( dataNLP.data.Xscale, M, 1 );
            auxdata.XscaleMat=diag(XscaleMat(:));
            auxdata.XshiftMat=repmat( dataNLP.data.Xshift, M, 1 );
            UscaleMat=1./repmat( dataNLP.data.Uscale, M, 1 );
            auxdata.UscaleMat=diag(UscaleMat(:));
            auxdata.UshiftMat=repmat( dataNLP.data.Ushift, M, 1 );
            if isfield(dataNLP.data,'Pscale')
