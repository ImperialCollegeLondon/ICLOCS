function jac=resMin_jacobianFD_ModeMinRes(L,E,g,avrc,X,Xr,U,Ur,P,T,b,x0,xf,u0,uf,p,t0,tf,data)
 ...
 for i=i_st:i_end
    if (nt && i>(nt+np)) || (~nt)
        for j=1:size(data.idx_perturb,2)
            [~,ResCost_p]=costResidualMin_ModeMinRes( X+ex{i}.*data.idx_perturb(:,j)*e,U+eu{i}.*data.idx_perturb(:,j)*e,P+ep{i}*e,(tf+etf{i}*e-t0-et0{i}*e)*T+t0+et0{i}*e,data);
            [~,ResCost_m]=costResidualMin_ModeMinRes( X-ex{i}.*data.idx_perturb(:,j)*e,U-eu{i}.*data.idx_perturb(:,j)*e,P-ep{i}*e,(tf-etf{i}*e-t0+et0{i}*e)*T+t0-et0{i}*e,data);
        ...
        [~,ResCost_p]=costResidualMin_ModeMinRes( X,U,P+ep{i}*e,(tf+etf{i}*e-t0-et0{i}*e)*T+t0+et0{i}*e,data);
