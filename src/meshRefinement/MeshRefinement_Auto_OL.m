function [ options ] = MeshRefinement_Auto_OL( options, data, solution, minItervalScale )
% MeshRefinement_Auto - Conduct mesh refinement for hp typed discretization
% methods (prioritize MR efficiency)
%
%
% Syntax:  [ options ] = MeshRefinement_Auto( options, data, solution, minItervalScale )
%
% Inputs:
...
solutionT=solution.coll.T/(solution.coll.tf-solution.t0)*(solution.tf-solution.t0);
...
idxd=idx+1;
idxd(idxd>length(solutionT))=[];
T_remesh_div=solutionT(idxd);
...
for i=1:length(options.tau_segment)-1
    ...
    if any(Max_error(i,:)>options.discErrorTol) && mesh_AI_indicator(i)~=1
        T_remesh=solutionT(1+tau_seg_idx(i));
        tau_remesh= ((1-(-1))*T_remesh-(solutionT(1)+solution.tf))/(solution.tf-solutionT(1));
        ...
        p_inc=ceil(max(log10(Max_error(i,:)./options.discErrorTol)));
            ...
            elseif options.npsegment(segidx_remesh)<=npd_Max
                ...
                T_remesh_div=union(T_remesh_div,(solutionT(tau_seg_idx(i))+solutionT(1+tau_seg_idx(i)))/2);
            end
...
for i=1:length(T_remesh_div)
    ...
    tau_remesh= ((1-(-1))*T_remesh-(solutionT(1)+solution.tf))/(solution.tf-solutionT(1));
    tau_mininterval= options.mintimeinterval*minItervalScale/(solution.tf-solutionT(1))*(1-(-1));
