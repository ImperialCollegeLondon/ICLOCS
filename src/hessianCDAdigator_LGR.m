...
function hessian=hessianCDAdigator_LGR(L,f,g,X,U,P,T,E,b,x0,xf,u0,uf,p,t0,tf,fxx_adigator,data)

%  It evaluates the Hessian of the Lagrangian with Adigator
%
% Syntax:  hessian=hessianCDAdigator_LGR(L,f,g,X,U,P,T,E,b,x0,xf,u0,uf,p,t0,tf,fxx_adigator,data)
%
% Inputs:
...
for i=1:nfd
   for j=1:i
     if i<nfd-nt-np+1 && j<nfd-nt-np+1
         if ~isempty(fxx_adigator)
            fxx=bsxfun(@times, fxx_adigator{i,j}', data.t_segment_end);
            fzz=fzz+sparse(idx(:,i),idx(:,j),reshape(fxx.*adjoint_f,M*n,1),nz,nz);
         end
     else
         if j==i
...
for i=1:nfd
   for j=1:i
    if j==i;
     ...
     gp2=g(X-ex{j}*e,U-eu{j}*e,P-ep{j}*e,(DT-etf(i)+et0(i))/2.*T+(k0-etf(i)-et0(i))/2,vdat);
...
for j=1:i
  if j==i;
    Lo=DT*L(X,Xr,U,Ur,P,DT/2*T+k0/2,vdat);
    Lp1=(DT+dtf_1-dt0_1)/2*L(X+dx1,Xr,U+du1,Ur,P+dp1,(DT+dtf_1-dt0_1)/2*T+(k0+dtf_1+dt0_1),vdat);
    Lp2=(DT-dtf_1+dt0_1)/2*L(X-dx1,Xr,U-du1,Ur,P-dp1,(DT-dtf_1+dt0_1)/2*T+(k0-dtf_1-dt0_1),vdat);
    Lt=(Lp2-2*Lo+Lp1).*data.map.w/e2;
  else
    dt0_2=e*et0{j};dtf_2=e*etf{j};dp2=e*ep{j};dx2=e*ex{j}; du2=e*eu{j};
    Lpp=(DT+dtf_1-dt0_1+dtf_2-dt0_2)/2*L(X+dx1+dx2,Xr,U+du1+du2,Ur,P+dp1+dp2,(DT+dtf_1-dt0_1+dtf_2-dt0_2)/2*T+(k0+dtf_1+dt0_1+dtf_2+dt0_2)/2,vdat);
    Lpm=(DT+dtf_1-dt0_1-dtf_2+dt0_2)/2*L(X+dx1-dx2,Xr,U+du1-du2,Ur,P+dp1-dp2,(DT+dtf_1-dt0_1-dtf_2+dt0_2)/2*T+(k0+dtf_1+dt0_1-dtf_2-dt0_2)/2,vdat);
    Lmp=(DT-dtf_1+dt0_1+dtf_2-dt0_2)/2*L(X-dx1+dx2,Xr,U-du1+du2,Ur,P-dp1+dp2,(DT-dtf_1+dt0_1+dtf_2-dt0_2)/2*T+(k0-dtf_1-dt0_1+dtf_2+dt0_2)/2,vdat);
    Lmm=(DT-dtf_1+dt0_1-dtf_2+dt0_2)/2*L(X-dx1-dx2,Xr,U-du1-du2,Ur,P-dp1-dp2,(DT-dtf_1+dt0_1-dtf_2+dt0_2)/2*T+(k0-dtf_1-dt0_1-dtf_2-dt0_2)/2,vdat);
