...
if (simtime~=0 && ~re_opt) || mod(simtime,tstep)~=0 % For time instances without re-optimization
        ...
        crt_input=[speval(solution,'U',1,crt_time);speval(solution,'U',2,crt_time);speval(solution,'U',3,crt_time) ...] % for mutiple inputs, change as needed
        tfpu=[solution.tf-crt_time;0;crt_input] ; % Return the interpolated solution
else
    options= settings_func(N);                  % Get options and solver settings 
    if simtime~=0 % For later instances
        ...
        if solution.tf > opt_st
            ...
            crt_input=[speval(solution,'U',1,opt_st) speval(solution,'U',2,opt_st) speval(solution,'U',3,opt_st) ... ] % for mutiple inputs, change as needed
            guess.inputs=[crt_input;solution.U(idx_new,:)];
        else
            guess.states(1,:)=x0';
        end
    
        % For shrinking horizon control only
    ...
    if simtime~=0
        ...
        crt_time=simtime-solution.t_ref; % Current time in the frame of previous solution
        crt_input=[speval(solution,'U',1,opt_st) speval(solution,'U',2,opt_st) speval(solution,'U',3,opt_st) ... ] % for mutiple inputs, change as needed
        tfpu=[solution.tf-crt_time;computation_time;crt_input] ; % Return the interpolated solution
    else
