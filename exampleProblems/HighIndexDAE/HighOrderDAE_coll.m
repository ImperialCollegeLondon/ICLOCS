function [problem,guess] = HighOrderDAE_coll
%HighOrderDAE - High Order DAE problem
%
...
InternalDynamics=@HighOrderDAE_Dynamics_Internal;
SimDynamics=@HighOrderDAE_Dynamics_Sim;
...
% Settings file
problem.settings=@settings_HighOrderDAE_coll;
...
problem.states.x0=[0 0 1 0];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[0 0 1 0]; 
problem.states.x0u=[0 0 1 0]; 
...
problem.inputs.urConstraintTol=[0.01 0.01];
...
% stageCost = u(:,2).^2+(x(:,1)-vdat.L.*sin(t+vdat.a)).^2+(x(:,3)-vdat.L.*cos(t+vdat.a)).^2;
stageCost = 0.0001*u(:,2).^2+(x(:,1)-vdat.L.*sin(t+vdat.a)).^2+(x(:,3)-vdat.L.*cos(t+vdat.a)).^2;
