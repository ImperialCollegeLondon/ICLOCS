...
function [df,dg,db]=jacConst_BangBang(f,g,X,U,P,t,b,x0,xf,u0,uf,p,tf,t0,data)
...
df.dt{1}=[0*t 0*t];             % Derivatives of f(x,u,p,t) wrt. t
% df.dt=[];

df.dx{1}=[0*Lt, 0*Lt]; % Derivatives of f(x,u,p,t) wrt. x1
df.dx{2}=[Lt, 0*Lt]; % Derivative of f(x,u,p,t) wrt. x2

df.du{1}=[0*Lt, Lt];  % Derivative of f(x,u,p,t) wrt. u1
