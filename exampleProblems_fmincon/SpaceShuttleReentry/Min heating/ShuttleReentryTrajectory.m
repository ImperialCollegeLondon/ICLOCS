...
problem.states.xrl=[-inf -inf -inf -inf -rate_const -rate_const];
problem.states.xru=[inf inf inf inf rate_const rate_const];
...
problem.states.xrConstraintTol=[1 deg2rad(0.5) deg2rad(0.5) 0.1 deg2rad(0.5) deg2rad(0.5)];

% Terminal state bounds. xfl=< xf <=xfu
problem.states.xfl=[h_f phi_min theta_f v_f gamma_f psi_min]; 
problem.states.xfu=[h_f phi_max theta_f v_f gamma_f psi_max];
...
% Input rate bounds
problem.inputs.url=[-inf -inf]; 
problem.inputs.uru=[inf inf]; 

% Input constraint error bounds
problem.inputs.uConstraintTol=[deg2rad(0.5) deg2rad(0.5)];
problem.inputs.urConstraintTol=[deg2rad(0.5) deg2rad(0.5)];
...
problem.data.c0=1.0672181;
problem.data.c1=-0.19213774e-01;
problem.data.c2=0.21286289e-03;
problem.data.c3=-0.10117249e-05;
