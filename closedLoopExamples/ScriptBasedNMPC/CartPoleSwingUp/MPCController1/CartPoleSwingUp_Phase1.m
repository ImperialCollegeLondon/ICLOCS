function [problem,guess] = CartPoleSwingUp_Phase1
...
InternalDynamics=@CartPoleSwingUp_Dynamics_Internal_Phase1;
SimDynamics=@CartPoleSwingUp_Dynamics_Sim_Phase1;

% Analytic derivative files (optional)
problem.analyticDeriv.gradCost=@gradCost_CartPoleSwingUp_Phase1;
problem.analyticDeriv.hessianLagrangian=@hessianLagrangian_CartPoleSwingUp_Phase1;
problem.analyticDeriv.jacConst=@jacConst_CartPoleSwingUp_Phase1;

% Settings file
problem.settings=@settings_CartPoleSwingUp_Phase1;
...
problem.states.xfl=[-inf -inf -2 -2*pi]; 
problem.states.xfu=[inf inf 2 2*pi];
...
problem.data.m2 = 0.3;
problem.data.L  = 0.5;
problem.data.tfmax=problem.time.tf_max;
...
% boundaryCost=100*(max(tf-vdat.tfmax,0)).^2;
boundaryCost=100*xf(1).^2+100*xf(2).^2+100*(xf(3)-1).^2+100*(xf(4)-pi).^2;
%------------- END OF CODE --------------
